from flask import Blueprint, redirect, url_for, render_template, current_app, request, flash, jsonify
...
def google_callback():
    ...
    if not email:
        flash("Não foi possível obter o email do Google", "danger")
        return redirect(url_for("auth.login"))
...
def my_profile():
    ...
    if changed_at:
        ...
        password_age_days = (date.today() - changed_at).days
    else:
...
def upload_avatar():
    ...
    try:
        url = save_profile_image(current_user.id, file)
        return jsonify({"url": url})
    except ValueError as e:
        return jsonify({"error": str(e)}), 400

@bp.route("/avatar/remove", methods=["POST"])
@login_required
def remove_avatar():

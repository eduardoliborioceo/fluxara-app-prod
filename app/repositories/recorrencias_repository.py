        ...
        with conn.cursor(row_factory=dict_row) as cur:
            cur.execute("""
                INSERT INTO recorrencias
                    (user_id, nome, tipo, valor, categoria_id, conta_id, cartao_id,
                     dia_vencimento, hora_execucao)
        ...
        with conn.cursor() as cur:
            cur.execute("""
                ...
                SET nome = %s, valor = %s, categoria_id = %s, conta_id = %s,
                    cartao_id = %s, dia_vencimento = %s, hora_execucao = %s
                WHERE id = %s AND user_id = %s
            """, (nome, valor, categoria_id, conta_id, cartao_id, dia_vencimento,
                  hora_execucao, recorrencia_id, user_id))
        conn.commit()
